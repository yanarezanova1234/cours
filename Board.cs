...
namespace WinFormsApp1
{
    public class Board
    {
        ...
        */
        ...
        // Создание списков шашек для белых и черных
        public List<Checker> lstOfWhiteChecker = new List<Checker>();
        public List<Checker> lstOfBlackChecker = new List<Checker>();
        public bool IsStartMove = false;// Флаг для определения начала хода
        public Move currentMove = new Move();// Текущий ход
        // Словарь клеток доски
        ...
        public Move ComputerMove() // Основной метод, содержащий распределение функциональности
        {
            ...
            GetListOfMoves(listOfBlackMoves, lstOfBlackChecker, BLACK);// Получение списка возможных ходов для черных шашек
            if (listOfBlackMoves.Count > 0)
            {
                Estimate(listOfBlackMoves);// Оценка ходов и выбор лучшего хода
        ...
        // Метод для имитации выполнения хода
        public void MoveImitation(Move curMove, List<Checker> lstOfCheckers)
        {
            ChangeCheckerLocation(curMove.MoveFrom, curMove.MoveTo, lstOfCheckers);// Имитация перемещения шашки
            ChangeDictOfCellsAfterMove(curMove.MoveFrom, curMove.MoveTo, curMove.ColorOfMovedChecker);// Изменение словаря клеток после хода
            if (curMove.IsAttack) // Если был выполнен захват шашки, удалить её из списка
        ...
        // Метод для получения списка шашек по их расположению
        ...
        // Метод для получения списка шашек по цвету
        ...
        // Метод для получения списка возможных ходов для шашек определенного цвета
        ...
        { // Обязательно декомпозировать метод в будущем, когда он устаканится
        ...
        // Метод для оценки ходов и выбора лучшего хода
        ...
        // Метод для выбора лучшего хода из списка возможных ходов
        ...
        // Метод для поиска хода с минимальными потерями
        ...
        { /*
            ...
            */
            CalculateCombinations(lstOfMoves, new List<Move>());// рекурсивно генерирует комбинации возможных ходов и оценивает их выгоду
            Combination combinationWithMinLoss = GetCombinationWithMinLoss(lstOfMoves[0].ColorOfMovedChecker);//возвращает комбинацию с наименьшим ущербом для игрока
            if (combinationWithMinLoss.combinationOfMoves.Count != 0)
                foreach (Move move in lstOfMoves)
                {
                    ...
                    {
                        move.EstimateOfProfit += 0.5; //Если такая комбинация найдена, оценка прибыли выбранного хода увеличивается на 0.5.
                    }
                }
        }

        // Метод для поиска комбинации с минимальным ущербом
        private Combination GetCombinationWithMinLoss(string color)
        {
            ...
            foreach (Combination curCombination in lstOfCombinations)//перебор всех комбинаций ходов из списка lstOfCombinations
            {
                if (color == WHITE)
        ...
        private void GetMovesOfChecker(string locationFrom, string checkerColor, List<Move> lstOfMoves) // Получение ходов конкретной шашки
        {
            ...
            Point tempEndLocationPoint = new Point();//временное хранение возможных конечных позиций шашки
            for (int x = moveFromPoint.X - 1; x <= moveFromPoint.X + 1; x += 2)
            {
                for (int y = moveFromPoint.Y - 1; y <= moveFromPoint.Y + 1; y += 2)
                {
                    ...
                    if (IsOnBoard(tempEndLocationPoint)) // если находится на игровом поле
                    {
...
*/
        ...
        private bool IsOnBoard(Point curPoint) 
        {
            bool flag = false;
